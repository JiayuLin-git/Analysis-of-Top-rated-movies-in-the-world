...
def get_movie_details(tmdbId, df_MovieLens):
    ...
    if tmdbId:
        ...
        MLens_rating_Series = df_MovieLens[df_MovieLens['tmdbId'] == 278]['rating']
    ...
    return [country, genres, budget, revenue, keywords, MLens_rating]
...
def proc_MovieLens(save=True):
    ...
    iddf = pd.read_csv('MovieLens/links.csv')
    df2 = pd.merge(df, iddf, how='left', on='movieId')

    df3 = pd.read_csv('MovieLens/ratings.csv')
...
def get_IMDB(source='',save=True):
    ...
    if save:
        if not os.path.exists('raw_data'):
            os.mkdir('raw_data')
        if source == 'test':
            df_movies.to_csv('./raw_data/IMDB_Top_250_raw_test.csv')
            print('Raw testing IMDB dataset got')
        else:
            df_movies.to_csv('./raw_data/IMDB_Top_250_raw.csv' )
...
def get_Douban(source='',save=True):
    ...
    if save:
        if not os.path.exists('raw_data'):
            os.mkdir('raw_data')
        if source == 'test':
            df_movies.to_csv('./raw_data/Douban_Top_250_raw_test.csv')
            print('Raw testing Douban dataset got')
        else:
            df_movies.to_csv('./raw_data/Douban_Top_250_raw.csv')
...
def get_Yahoo(source='',save=True):
    ...
    if save:
        if not os.path.exists('raw_data'):
            os.mkdir('raw_data')
        if source == 'test':
            pd_data.to_csv('./raw_data/Yahoo_Top_500_raw_test.csv')
            print('Raw testing Yahoo dataset processed')
        else:
            pd_data.to_csv('./raw_data/Yahoo_Top_500_raw.csv')
...
def proc_IMDB(df_IMDB_raw, df_MovieLens,source=''):
    ...
    df_IMDB = pd.DataFrame(lists_IMDB,
                           columns=['rank', 'name', 'year', 'IMDB_rating', 'country', 'genres', 'budget', 'revenue',
                                    'keywords', 'MLens_rating'])
    if source == 'test':
        df_IMDB.to_csv('IMDB_top_250_test.csv')
        print('IMDB testing dataset processed')
    else:
        df_IMDB.to_csv('IMDB_top_250.csv')
...
def proc_Douban(df_Douban_raw, df_MovieLens,source=''):
    ...
    df_Douban = pd.DataFrame(lists_Douban,
                             columns=['rank', 'name', 'year', 'Douban_rating', 'country', 'genres', 'budget', 'revenue',
                                      'keywords', 'MLens_rating'])
    if source == 'test':
        df_Douban.to_csv('Douban_top_250_test.csv')
        print('Douban testing dataset processed')
    else:
        df_Douban.to_csv('Douban_top_250.csv')
...
def proc_Yahoo(df_Yahoo_raw, df_MovieLens,source=''):
    ...
    df_Yahoo = pd.DataFrame(lists_Yahoo,
                            columns=['rank', 'name', 'year', 'Yahoo_rating', 'country', 'genres', 'budget', 'revenue',
                                     'keywords', 'MLens_rating'])
    if source=='test':
        df_Yahoo.to_csv('Yahoo_top_500_test.csv')
        print('Yahoo testing dataset processed')
    else:
        df_Yahoo.to_csv('Yahoo_top_500.csv')
...
def main(source):
    if source == 'local':
        try:
            df_MovieLens = pd.read_csv('MovieLens.csv')
        ...
        try:
            df_Douban = pd.read_csv('Douban_top_250.csv')
        ...
        try:
            df_Yahoo = pd.read_csv('Yahoo_top_500.csv')
